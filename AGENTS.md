...
- `src/utils/telemetry_manager.py` + `src/config/telemetry.py`: OpenTelemetry (OTLP) producer — provider/exporter lifecycle and the `defaults.telemetry` config model (disabled by default; see `docs/configuration/telemetry.md`)
...
- **Database incremental (JDBC)** — Resource-level `incremental_extract` (v1: `jdbc_companion_cdc` with a companion CDC table; `watermark.cursor` with `strategy` `destination_column`, `reference_column`, and optional `tolerance_calendar_days` / `reference_format`) is defined in `src/config/incremental_extract.py` and cross-validated on `ResourceConfig`. `watermark.cursor.strategy` `destination_column` reads `MAX(reference_column)` via `BaseLoadStrategy.read_max_column_as_string` (Delta and Iceberg); `append` or `merge` write modes are required, and `database_select_query` is not allowed. Optional `database_where_predicate` adds a main-table boolean (alias ``m`` in generated SQL); it is mutually exclusive with `database_select_query` and composes with `table_read_options.predicates` via one inner ``WHERE``. On **warm** incremental JDBC reads, `table_read_options.use_on_incremental_warm` defaults false so predicates and range partitioning are omitted unless set true; cold runs always honor parallel options. Runtime: `DynamicHandler._build_database_dataframe`, `src/service/jdbc_companion_incremental.py`, `src/handler/incremental_cursor.py`, and `jdbc_dbtable_from_plain_table` in `src/service/sql_database_service.py`. Operator-oriented `correlation` and run behavior: `docs/configuration/database-incremental.md`.
...
- **Telemetry (OpenTelemetry)** — Spine is a backend-agnostic OTLP **producer**: it emits standard OTLP traces/metrics and defers to standard `OTEL_*` env vars. Add a new signal or setting by extending `TelemetryConfig` (`src/config/telemetry.py`) and `TelemetryManager` (`src/utils/telemetry_manager.py`) **together**. Never hardcode collector endpoints, storage formats, or vendor-specific attributes (product, workflow, business-event, cost, `gen_ai.*`, …) — operators supply those through the free-form `resource_attributes` map or `OTEL_RESOURCE_ATTRIBUTES`. Instrument at existing flow boundaries via the `traced` decorator or `self._tracer`; do not add `if telemetry_enabled:` guards (the OTEL API is a no-op when disabled). Telemetry must never break ingestion: an unreachable/missing endpoint degrades to no-op with a warning rather than failing the run (a deliberate exception to fail-early).
