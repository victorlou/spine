...
- **`use_on_incremental_warm`** (default **`false`**): For resources with **`incremental_extract`**, **warm** JDBC reads (after a destination cursor exists) skip **`predicates`** and **range partitioning** by default so the bounded read uses one Spark JDBC partition; set to **`true`** to keep parallel reads on warm runs. **`fetch_size`** is always honored. **Cold** incremental loads always use predicates/range as configured. Details: [Database incremental extract (JDBC)](database-incremental.md).
...
Future JDBC-backed sources (for example MySQL or Redshift) can reuse this block once they use the same Spark read path. See commented examples in [config/examples/postgres.example.yml](https://github.com/victorlou/spine/blob/main/config/examples/postgres.example.yml).
