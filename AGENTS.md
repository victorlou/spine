...
- **Database incremental (JDBC)** — Resource-level `incremental_extract` (v1: `jdbc_companion_cdc` with a companion CDC table; `watermark.cursor` with `strategy` `destination_column`, `reference_column`, and optional `tolerance_calendar_days` / `reference_format`) is defined in `src/config/incremental_extract.py` and cross-validated on `ResourceConfig`. It requires Delta `loading.format` for `MAX` cursor reads, `append` or `merge` write modes, and cannot be combined with `database_select_query`. Optional `database_where_predicate` adds a main-table boolean (alias ``m`` in generated SQL); it is mutually exclusive with `database_select_query` and composes with `table_read_options.predicates` via one inner ``WHERE``. Runtime: `DynamicHandler._build_database_dataframe`, `src/service/jdbc_companion_incremental.py`, `src/handler/incremental_cursor.py`, and `jdbc_dbtable_from_plain_table` in `src/service/sql_database_service.py`. Operator-oriented `correlation` and run behavior: `docs/configuration/database-incremental.md`.
